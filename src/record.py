...
class Recorder:
    ...
    def compress_and_upload(self, folder_name, format):
        ...
        if self.parameters["compute_chemotaxis"]:
            self.compute_chemotaxis(video_path=compressed_file)
...
### Other utility functions
    ...
    def compute_chemotaxis(self, video_path):
        ...
        cap, width, height = load_video(video_path)

        # Read the first frame
        ret, first_frame = cap.read()
        first_frame = get_specific_frame(video_path, 3)
        ...
        # Remove background
        positions = locate_worms(video_path)

        # Compute chemotaxis index
        chemotaxis_index_by_frame = compute_chemotaxis_index(positions, middle)

        # Save centers of mass to CSV file
        # save_centers_to_csv(positions, 'centers.csv')

        # Save chemotaxis indices to CSV file
        save_chemotaxis_indices_to_csv(chemotaxis_index_by_frame, 'chemotaxis_indices.csv')

        # Plot chemotaxis index
        plot_chemotaxis_index(chemotaxis_index_by_frame)
...
def detect_circles(binary_frame, max_distance_from_center=200):
...
def locate_worms(video_path, detected_circles=None, min_element_area_threshold=30, stop_at_frame=50):
    # Open the video file
    cap = cv2.VideoCapture(video_path)
...
def compute_chemotaxis_index(all_centers_by_frame, middle):
    ...
    for frame_index, centers_of_mass in all_centers_by_frame.items():
        ...
        print(f"Frame {frame_index}: {left_points} points on the left, {right_points} points on the right, chemotaxis index = {chemotaxis_index}")
...
def save_centers_to_csv(centers_list, csv_filename):
    """
...
def save_chemotaxis_indices_to_csv(chemotaxis_indices, csv_filename):
    """
...
def plot_chemotaxis_index(chemotaxis_index_by_frame, frame_rate=2, output_prefix="chemotaxis_plot", font_size=14):
    ...
    chemotaxis_values = [chemotaxis_index_by_frame[frame_index] for frame_index in sorted(chemotaxis_index_by_frame.keys())]
